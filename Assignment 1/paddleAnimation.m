...
massSide = 0.001;
massObj = CubeClass([0.3, massSide]);
...
%     springObj.globalMove(SE3([0, 0, 0, 0, 0, pi/2]));
%     springObj.updateState(SE3([0, paddle_pos - puck_r - p.l0, 0, 0, 0, pi/2]), ...
%         p.l0+x_pos);
    springObj.updateState(SE3([0, paddle_pos - puck_r - p.l0, 0, 0, 0, pi/2]), ...
        p.l0);
    ...
    % Mass position
    massObj.resetFrame
    massObj.globalMove(SE3([0 paddle_pos - puck_r + massSide/2 0]));
    
%     paddleObj.globalMove(SE3([0, -p.l0-x_pos-massSide/2, 0]))
    paddleObj.globalMove(SE3([0, -p.l0-massSide/2, 0]))
